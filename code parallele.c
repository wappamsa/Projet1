...
int code1();
void diffusion(void* data, int count, MPI_Datatype datatype, int root,
...
int main(int argc, char** argv) 
    {
        ...
        if(world_rank == 0){
            data = serial();
            diffusion(&data, world_size, MPI_INT, 1, MPI_COMM_WORLD);
        }
        else{
            diffusion(&data, world_size, MPI_INT, 0, MPI_COMM_WORLD);
        }
...
void diffusion(void* data, int count, MPI_Datatype datatype, int root,
    ...
    if(world_rank == root){
        ...
        MPI_Recv(data, count, datatype, root, 0, communicator, MPI_STATUT_IGNORE);
    }
}
